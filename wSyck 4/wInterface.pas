  ...
  try
    ...
    begin
      ...
      begin
        ...
        RefItem(Count.RefSlot, ItemRefToEffect(4, StrToInt(Temp[1])), StrToInt(Temp[2]));
        CliMsg(1000, PAnsiChar(AnsiString('Refinando Item +'+IntToStr(ItemEffectToRef(4, Equip[StrToInt(Temp[1])].EFV1))+', Aguarde ...')));
        Inc(Count.RefSlot);
        Inc(Count.RefNum);
        ...
        ShowMsg(PAnsiChar(AnsiString('PLS: '+IntToStr(Count.RefNum))));
      end;
...
begin
  try
    ...
    begin
      ...
      begin
        Inc(Numeric.Pos);
        CliMsg(3000, PAnsiChar(AnsiString('Testando Senha '+LeftValue(Numeric.Pos, Numeric.Size)+'/'+LeftValue(Numeric.Max, Numeric.Size)+', Aguarde ...')));
        if Numeric.Pos >= Numeric.Max then
          Numeric.Active := False;
      end else
      begin
        Dec(Numeric.Pos);
        CliMsg(3000, PAnsiChar(AnsiString('Testando Senha '+LeftValue(Numeric.Pos, Numeric.Size)+'/'+LeftValue(Numeric.Min, Numeric.Size)+', Aguarde ...')));
