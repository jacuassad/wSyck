...
uses
  ...
  procedure ShowMsg(Msg : PANSICHAR);
  procedure CliMsg(Delay : INTEGER; Msg : PANSICHAR);
  {FUNÇÕES DE DADOS}
  ...
  {PACKET CONTROL}
  procedure PacketControl(Data, dLength : Pointer; dType : Integer);
  {PROMPT}
...
var
  PtrEquip : ^TEQUIP;
begin
  PtrEquip := @PDWORD(PDWORD($00612AC4)^+$8C)^;
  Result := PtrEquip^;
end;
...
begin
  PtrInventory := @PDWORD(PDWORD($00612AC4)^+$10C)^;
  Result := PtrInventory^;
end;
...
var
  PtrCargo : ^TCARGO;
begin
  PtrCargo := @PDWORD(PDWORD($0)^+$0)^; {NÃO IMPLEMENTADO}
  Result := PtrCargo^;
end;
...
begin
  ...
  begin
    VirtualProtect(PTR($00496FFB), 10, PAGE_EXECUTE_WRITECOPY, RET);
    PDWORD(PDWORD($00440C84)^+$7FB)^ := $FF99FF22;
    ShowGameMsg(Msg);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        CliMsg(3000, PAnsiChar(AnsiString('Move X = '+KEY[1]+' Y = '+KEY[2])));
      end else
      ...
      begin
        ...
        CliMsg(3000, PAnsiChar(AnsiString('Buy : NPC = '+KEY[1]+' Slot = '+KEY[2]+' Qtd. = '+KEY[3])));
      end else

      if KEY[0] = 'clicknpc' then
      begin
        ClickNPC(StrToInt(KEY[1]), StrToInt(KEY[2]));
        CliMsg(3000, PAnsiChar(AnsiString('Click : NPC = '+KEY[1]+' Qtd. = '+KEY[2])));
      end else
      ...
      begin
        ...
        CliMsg(3000, PAnsiChar(AnsiString('Sell : NPC = '+KEY[1]+' Items = '+Temp)));
      end else
      ...
      begin
        Temp := Copy(CMD, Length(KEY[0])+2, Length(CMD)-1);
        Amount(Temp);
        CliMsg(3000, PAnsiChar(AnsiString('Agrupar Items : '+Temp)));
      end else
      ...
      begin
        Temp := Copy(CMD, Length(KEY[0])+2, Length(CMD)-1);
        ReAmount(Temp);
        CliMsg(3000, PAnsiChar(AnsiString('Desagrupar Items : '+Temp)));
      end else
      ...
      begin
        Party(KEY[1]);
        CliMsg(3000, PAnsiChar(AnsiString('Grupo Nick = '+KEY[1]+', Index = '+IntToStr(PlayerID))));
      end else
      ...
      begin
        Temp := Copy(CMD, Length(KEY[0])+2, Length(CMD)-1);
        Chat(Temp);
        CliMsg(3000, PAnsiChar(AnsiString('Chat Index = '+IntToStr(PlayerID)+', diz : '+Temp)));
      end else
      ...
      begin
        ...
        begin
          Sec.X := PosX;
          Sec.Y := PosY;
          CliMsg(3000, PAnsiChar(AnsiString('Sec [ON] X = '+IntToStr(Sec.X)+' Y = '+IntToStr(Sec.Y))));
        end else
        begin
          Sec.X := 0;
          Sec.Y := 0;
      ...
      if KEY[0] = 'equip' then
      begin
        ShowMsg(PAnsiChar(AnsiString('Equip Slot = '+KEY[1])));
        ShowMsg(PAnsiChar(AnsiString('ItemID = '+IntToStr(Equip[StrToInt(KEY[1])].ID))));
        ShowMsg(PAnsiChar(AnsiString('EF1 = '+IntToStr(Equip[StrToInt(KEY[1])].EF1)+', '+'EFV1 = '+IntToStr(Equip[StrToInt(KEY[1])].EFV1))));
        ShowMsg(PAnsiChar(AnsiString('EF2 = '+IntToStr(Equip[StrToInt(KEY[1])].EF2)+', '+'EFV2 = '+IntToStr(Equip[StrToInt(KEY[1])].EFV2))));
        ShowMsg(PAnsiChar(AnsiString('EF3 = '+IntToStr(Equip[StrToInt(KEY[1])].EF3)+', '+'EFV3 = '+IntToStr(Equip[StrToInt(KEY[1])].EFV3))));
      end else

      if KEY[0] = 'inventory' then
      begin
        ShowMsg(PAnsiChar(AnsiString('Inventory Slot = '+KEY[1])));
        ShowMsg(PAnsiChar(AnsiString('ItemID = '+IntToStr(Inventory[StrToInt(KEY[1])].ID))));
        ShowMsg(PAnsiChar(AnsiString('EF1 = '+IntToStr(Inventory[StrToInt(KEY[1])].EF1)+', '+'EFV1 = '+IntToStr(Inventory[StrToInt(KEY[1])].EFV1))));
        ShowMsg(PAnsiChar(AnsiString('EF2 = '+IntToStr(Inventory[StrToInt(KEY[1])].EF2)+', '+'EFV2 = '+IntToStr(Inventory[StrToInt(KEY[1])].EFV2))));
        ShowMsg(PAnsiChar(AnsiString('EF3 = '+IntToStr(Inventory[StrToInt(KEY[1])].EF3)+', '+'EFV3 = '+IntToStr(Inventory[StrToInt(KEY[1])].EFV3))));
      end else
      ...
      begin
        ...
        FakeItem(Item);
        CliMsg(3000, PAnsiChar(AnsiString('FakeItem : '+IntToStr(Item.ID)+' '+IntToStr(Item.EF1)+' '+ IntToStr(Item.EFV1)+' '+IntToStr(Item.EF2)+' '+ IntToStr(Item.EFV2)+' '+IntToStr(Item.EF3)+' '+ IntToStr(Item.EFV3))));
        ShowMsg(PAnsiChar(AnsiString('FakeItem: '+IntToStr(Item.ID))));
      end else
